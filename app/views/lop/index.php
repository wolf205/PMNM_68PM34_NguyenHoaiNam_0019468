...
<body>
    <div class="container">
        ...
        <a href="/lop/create" class="btn-add">Add Class</a>
            ...
            <tbody>
                <?php if (!empty($data)): ?>
                    <?php foreach ($data as $lop): ?>
                        <tr>
                            <td><?php echo $lop['ma_lop']; ?></td>
                            <td><?php echo $lop['ten_lop']; ?></td>
                            <td><?php echo $lop['ghi_chu']; ?></td>
                            <td>
                                <a href="/lop/edit/<?php echo $lop['ma_lop'] ?>" class="btn-action btn-edit">Sửa</a>
                                <a href="/lop/delete/<?php echo $lop['ma_lop'] ?>" class="btn-action btn-delete" onclick="return confirm('Bạn có chắc chắn muốn xóa lớp này?')">Xoá</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="4" class="no-data">Chưa có dữ liệu lớp học nào.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
            ...
            <?php if (!empty($totalPage)): ?>
                <div class="pagination">
                    <?php for ($i = 1; $i <= $totalPage; $i++): ?>
                        <a href="/lop/index/<?php echo $i; ?>" class="page-link"><?php echo $i; ?></a>
                    <?php endfor; ?>
                </div>
            <?php endif; ?>
        </div>
